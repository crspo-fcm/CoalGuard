...
function severityLabel(value: unknown): "Critical" | "High" | "Moderate" {
...
function severityClass(value: unknown) {
  ...
  if (label === "Critical") return "bg-red-500/10 text-red-400";
  if (label === "High") return "bg-orange-500/10 text-orange-400";
...
function AlertsCenter() {
  const [violations, setViolations] = useState<BackendViolation[]>([]);
  ...
  const [severityFilter, setSeverityFilter] =
    useState<"All" | "Critical" | "High" | "Moderate">("All");

  const [expandedId, setExpandedId] = useState<number | null>(null);
  const [savingId, setSavingId] = useState<number | null>(null);
  const [deletingFraudId, setDeletingFraudId] = useState<number | null>(null);
  ...
  const loadViolations = async (): Promise<BackendViolation[]> => {
  ...
  const loadData = async (showLoading = false) => {
    try {
      ...
      const [nextViolations, nextFraudAlerts] = await Promise.all([
        loadViolations(),
        getFraudAlerts(),
      ]);

      setViolations(nextViolations);
      setFraudAlerts(nextFraudAlerts);
      setBackendError("");
      setLastUpdated(new Date().toLocaleTimeString("en-IN"));
    } catch (error) {
      console.error("Violations management backend error:", error);
      setBackendError(
  ...
  const openViolations = useMemo(
    () => violations.filter((v) => !isResolved(v.status)),
    [violations]
  );
  ...
  const criticalCount = openViolations.filter(
    (v) => severityRank(v.severity) === 4
  ).length;

  const highCount = openViolations.filter(
    (v) => severityRank(v.severity) === 3
  ).length;

  const moderateCount = openViolations.filter(
    (v) => severityRank(v.severity) <= 2
  ).length;
  ...
  const filteredViolations = useMemo(() => {
    return violations.filter((v) => {
      const statusMatch =
        filter === "All" ||
        (filter === "Open" && !isResolved(v.status)) ||
        (filter === "Resolved" && isResolved(v.status));

      const severityMatch =
        severityFilter === "All" ||
        severityLabel(v.severity) === severityFilter;

      return statusMatch && severityMatch;
    });
  }, [violations, filter, severityFilter]);

  const startEditing = (violation: BackendViolation) => {
    setExpandedId(violation.id);
    setEditForm({
      assigned_to: violation.assigned_to || "",
      corrective_action: violation.corrective_action || "",
      due_date: violation.due_date
        ? String(violation.due_date).slice(0, 10)
        : "",
  ...
  const saveViolation = async (violation: BackendViolation) => {
    try {
      ...
      if (editForm.status === "Resolved") {
        if (!payload.verified_by) {
          window.alert("Enter Verified By before resolving the violation.");
          return;
        }

        payload.verified_at = new Date().toISOString();
      } else {
        payload.verified_at = violation.verified_at || "";
      }

      if (
        editForm.status !== "Resolved" &&
        normalize(editForm.status) !== "awaiting verification"
      ) {
        payload.verified_by = editForm.verified_by.trim();
      }
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to update violation.");
      }

      if (data.violation) {
        setViolations((current) =>
          current.map((item) =>
            item.id === violation.id ? data.violation : item
          )
        );
      } else {
      ...
      window.alert(`Violation #${violation.id} updated successfully.`);
      setExpandedId(null);
    } catch (error) {
      window.alert(
        error instanceof Error
  ...
  const moveToNextStage = async (violation: BackendViolation) => {
    const current = normalize(violation.status);
    let next = "Reported";

    if (current === "reported") next = "In Progress";
    else if (current === "in progress") next = "Awaiting Verification";
    else if (current === "awaiting verification") next = "Resolved";
    else return;
    ...
    try {
      ...
      window.alert(`Violation #${violation.id} moved to ${next}.`);
    } catch (error) {
      window.alert(
  ...
  const handleDeleteFraudAlert = async (alertId: number) => {
    if (
      !window.confirm(
    ...
    try {
      setDeletingFraudId(alertId);
      await deleteFraudAlert(alertId);
      setFraudAlerts((current) => current.filter((a) => a.id !== alertId));
    } catch (error) {
      window.alert(
        error instanceof Error
          ? error.message
      ...
      setDeletingFraudId(null);
    }
  };

  const activeAlerts = openViolations.length + fraudAlerts.length;
  const totalCritical = criticalCount + fraudAlerts.length;
    ...
    <section className="mt-8 px-4 pb-12">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-slate-400">
          Review violations, assign corrective actions, track resolution and
          maintain an auditable management workflow.
        </p>
      </div>
      ...
      {backendError && (
        <div className="mb-5 rounded-xl border border-red-800/60 bg-red-950/20 p-4">
          <p className="font-bold text-red-400">Backend Error</p>
          <p className="mt-1 text-xs text-slate-400">{backendError}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5">
          ...
          <p className="mt-2 text-3xl font-bold text-white">{activeAlerts}</p>
          <p className="mt-1 text-xs text-slate-500">Violations + GPS alerts</p>
        </div>
        ...
        <div className="rounded-2xl border border-red-900/70 bg-red-950/20 p-5">
          ...
          <p className="mt-1 text-xs text-slate-500">Critical + GPS</p>
        </div>
        ...
        <div className="rounded-2xl border border-orange-900/70 bg-orange-950/20 p-5">
          ...
          <p className="mt-1 text-xs text-slate-500">High-risk violations</p>
        </div>
        ...
        <div className="rounded-2xl border border-yellow-900/70 bg-yellow-950/20 p-5">
          ...
          <p className="mt-1 text-xs text-slate-500">Moderate-risk violations</p>
        </div>
        ...
        <div className="rounded-2xl border border-green-900/70 bg-green-950/20 p-5">
          ...
          <p className="mt-1 text-xs text-slate-500">Closed violations</p>
        </div>
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-red-800/60 bg-red-950/10 p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Rejected inspection attempts outside the registered mine
              boundary.
            </p>
          </div>
        ...
        {fraudAlerts.length === 0 ? (
          ...
          <div className="mt-4 space-y-3">
            {fraudAlerts.slice(0, 10).map((fraud) => (
              <div
                key={fraud.id}
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-sm font-bold text-white">
                      {fraud.inspector_name || "Unknown Inspector"}
                    </p>
                    <p className="mt-1 text-xs text-slate-400">
                      {fraud.location_name || "Unknown Mine"}
                    </p>
                  ...
                  <div className="text-left md:text-right">
                    <p className="text-lg font-black text-red-400">
                      {Number(fraud.distance_meters).toFixed(1)} m
                    </p>
                    <button
                      type="button"
                      onClick={() => void handleDeleteFraudAlert(fraud.id)}
                      disabled={deletingFraudId === fraud.id}
      ...
      <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900 p-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h3 className="font-semibold text-white">Violation Queue</h3>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {(["All", "Critical", "High", "Moderate"] as const).map((item) => (
            <button
              key={item}
              type="button"
      ...
      <div className="mt-5 space-y-4">
        {loading && violations.length === 0 ? (
          ...
          filteredViolations.map((violation) => {
            const resolved = isResolved(violation.status);
              ...
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="text-left lg:text-right">
                    ...
                    <p className="mt-1 text-sm text-slate-300">
                      {getTimeText(violation.created_at)}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="rounded-xl bg-slate-950 p-4">
                    ...
                    <p className="mt-1 text-sm font-semibold text-white">
                      {violation.location_name || "Unknown Mine"}
                    </p>
                  </div>
                    ...
                    <>
                      <button
                        ...
                        onClick={() =>
                          expanded
                            ? setExpandedId(null)
                            : startEditing(violation)
                        }
                        className="rounded-lg bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-500"
                      >
                        {expanded ? "Close Manager Panel" : "Manage Violation"}
                      </button>

                      <button
                        type="button"
                        onClick={() => void moveToNextStage(violation)}
                        disabled={savingId === violation.id}
                      ...
                      >
                        {savingId === violation.id
                          ? "Updating..."
                          : normalize(violation.status) === "reported"
                          ? "Start Corrective Action"
                          : normalize(violation.status) === "in progress"
                ...
                {expanded && (
                  <div className="mt-5 rounded-2xl border border-blue-900/60 bg-slate-950 p-5">
                    <div className="mb-5">
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Changes are saved to SQLite and recorded in the Audit
                        Ledger by the backend.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      ...
                      <label className="block md:col-span-2">
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateField("corrective_action", e.target.value)
                          }
                      ...
                      <label className="block">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateField("verified_by", e.target.value)
                          }
                      ...
                      <label className="block md:col-span-2">
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateField("resolution_note", e.target.value)
                          }
                    ...
                    <div className="mt-5 flex flex-wrap justify-end gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => void saveViolation(violation)}
                        disabled={savingId === violation.id}
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-[11px] text-slate-600">
                      Reported by: {violation.reported_by || "System"}
                    </p>
                    {violation.verified_by && (
                      <p className="mt-1 text-[11px] text-slate-600">
                        Verified by: {violation.verified_by}
                      </p>
                    )}
                  </div>
      ...
      <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <h3 className="text-lg font-bold text-white">Escalation Workflow</h3>
        <p className="mt-1 text-sm text-slate-400">
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-4">
          {[
            ["LEVEL 1", "Non-Compliance Reported", "Inspector records a field observation."],
            ["LEVEL 2", "Management Review", "Manager reviews and assigns corrective action."],
            ["LEVEL 3", "Corrective Action", "Action progresses through verification."],
            ["LEVEL 4", "Resolution & Audit", "Resolution is verified and permanently logged."],
          ].map(([level, title, description], index) => (
            <div
              key={level}
            ...
            >
              <p className="text-xs font-bold text-blue-400">{level}</p>
